...
int main()
{
	...
	for (ciclo = 0; ciclo < 3; ciclo++)
	{
		...
		for (num = 0; num < totalvalores; num++){
			if(opcao==1) listaLSE = inserirLSE(listaLSE, vet[num]); 
			else listaLSE = inserirLSE(listaLSE, num);}

		for (num = 0; num <= totalvalores; num += 10){
			if(opcao==1) nodoLSE = consultarLSE(listaLSE, vet[num], &cmpsLSE);
			else nodoLSE = consultarLSE(listaLSE, num, &cmpsLSE);}
		...
		for (num = 0; num < totalvalores; num++){
			if(opcao==1) arvABP = inserirABP(arvABP, vet[num], &cmpsABP);
			else arvABP = inserirABP(arvABP, num, &cmpsABP);}

		for (num = 0; num <= totalvalores; num += 10){
			if(opcao==1) nodoABP = consultarABP(arvABP, vet[num], &cmpsABP);
			else nodoABP = consultarABP(arvABP, num, &cmpsABP);} 
		...
		for (num = 0; num < totalvalores; num++){
			if(opcao==1) arvAVL = inserirAVL(arvAVL, vet[num], &ok, &cmpsAVL);
			else arvAVL = inserirAVL(arvAVL, num, &ok, &cmpsAVL);}
			

		for (num = 0; num <= totalvalores; num += 10){
			if(opcao==1) nodoAVL = consultarAVL(arvAVL, vet[num], &cmpsAVL);
			else nodoAVL = consultarAVL(arvAVL, num, &cmpsAVL);}
